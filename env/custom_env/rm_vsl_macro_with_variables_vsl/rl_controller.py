...
class RLController(SumoEnv):
    """Variant: joint ramp metering + whole-cycle VSL on vsl_zone_0.

    Action space is 21 = 7 green-time choices x 3 VSL speed levels.
        green_idx = action % 7   -> green time in {10, 15, 20, 25, 30, 35, 40} s
        vsl_idx   = action // 7  -> VSL speed in {50, 75, 100} km/h

    VSL is applied to `vsl_zone_0` (rightmost mainline lane) for the FULL 40s
    control cycle — both during the green and red sub-phases — so that the
    upstream speed harmonization effect persists continuously.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        )  # 7 green time levels (10s → 40s)
        self.vsl_speed_actions_mps = np.array(
            [13.89, 20.83, 27.77]
        )  # 3 VSL levels: 50, 75, 100 km/h
        self.action_space_n = len(self.green_time_actions_sec) * len(
            self.vsl_speed_actions_mps
        )  # total 21 actions (7 x 3)

        self.green_phase_index = 0
        self.red_phase_index = 1

        # Induction loop detector IDs (sourced from 1ramp_1x3.add.xml)
        self.upstream_detector_ids = [
            "up_stream_sens_0",
            "up_stream_sens_1",
            "up_stream_sens_2",
        ]

        self.bottleneck_detector_ids = [
            "bottle_neck_sens_0",
            "bottle_neck_sens_1",
            "bottle_neck_sens_2",
            "bottle_neck_sens_3",
        ...
        # Initialize processed statistics to zero for the first observation after reset
    ...
    def _collect_data_at_cycle_end(self):
        self.processed_flow_upstream_vph = self.get_loops_flow_interval(
            self.upstream_detector_ids, self.CYCLE_DURATION_SEC
        )
        self.processed_flow_merging_vph = self.get_loops_flow_interval(
            self.bottleneck_detector_ids, self.CYCLE_DURATION_SEC
        )
        self.processed_mainline_flow_downstream_vph = self.get_loops_flow_interval(
            self.outflow_detector_ids, self.CYCLE_DURATION_SEC
        )
        ...
        self.processed_mainline_speed_downstream_mps = (
            self.get_loops_mean_speed_interval(self.outflow_detector_ids)
        )
        ...
        # Lane-1 (VSL lane0's continuation / rightmost)
        self.processed_flow_lane_0_merging_vph = self.get_loops_flow_interval(
            [self.bottleneck_detector_ids[1]], self.CYCLE_DURATION_SEC
        )
        self.processed_occ_lane_0_bottleneck_percent = (
            self.get_loops_occupancy_interval([self.bottleneck_detector_ids[1]])
        )
        self.processed_speed_lane_0_bottleneck_mps = self.get_loops_mean_speed_interval(
            [self.bottleneck_detector_ids[1]]
        )

        # Lane-0 (VSL lane / rightmost) — upstream detectors
        self.processed_flow_lane_0_upstream_vph = self.get_loops_flow_interval(
            [self.upstream_detector_ids[0]],
            self.CYCLE_DURATION_SEC,  # index 0 = lane 0
        )
    ...
    def step(self, action_index):
        ...
        # Apply VSL value for the full control cycle.
        ...
        if (
            ...
            num_steps_green = (
                int(round(chosen_green_time_sec / self.sim_step_length))
                if self.sim_step_length > 0
                else 0
            )
        ...
        if self.ramp_meter_id and self.red_phase_index != -1 and red_time_sec > 0:
            ...
            num_steps_red = (
                int(round(red_time_sec / self.sim_step_length))
                if self.sim_step_length > 0
                else 0
            )
        ...
        # Collect the road statistics of the current control cycle
        self._collect_data_at_cycle_end()
    ...
    # Observation Section ###############################################################################
    # Each observation component is normalized to [0,1] and
    # concatenated into a single state vector for the RL agent.
    ...
    # Reward Section ###############################################################################
    # Each reward component is normalized to [0,1] and weighted in the final reward calculation.
    ...
    # Gym API methods ##############################################################################
    # Required for compatibility with the OpenAI Gym interface.
